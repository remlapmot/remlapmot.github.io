...
Let's say we have prepared our practical in a Quarto document. In this case we'll use the actions and examples in the [quarto-dev/quarto-actions](https://github.com/quarto-dev/quarto-actions) repository.

My opinion is that running the practical once per month is about the sweetspot.

Recommend running on Windows, macOS, and maybe Linux. 

## GitHub Actions woes

At this point I acknowledge that sometimes GitHub Actions can be more trouble than they are worth because they can often fail for reasons which are not problems with your code. For example, I had one repository in which the Windows runner would regularly lose internet connection half way through installing the R dependency packages and hence would notify me of a failed check. Also you need to keep the code in your workflow file up-to-date. Usually a quick comparison back to the current version of the file in the r-lib/actions repo is enough to show you what you need to change.
