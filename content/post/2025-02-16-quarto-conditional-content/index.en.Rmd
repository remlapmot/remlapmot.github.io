---
title: "How to programmatically include conditional content for several Quarto engines"
...
tags:
  ...
  - Jupyter
subtitle: ''
summary: "How to programmatically include conditional content for several Quarto engines (`knitr`, `jupyter: python3`, `jupyter: nbstata`, and `engine: julia`) using parameters or environment variables to toggle inline code to write Quarto Markdown in the Quarto documents."
authors: []
...
Inline code has been a feature of knitr for a while. Yihui Xie and Christophe Dervieux have used it to pull off some fantastic tricks. My favourite trick is using it to programmatically write out R Markdown/Markdown code within an R Markdown document when a certain condition is met.
...
The problem I found was that after a first render the value of the environment variable seems to be cached and difficult to change on subsequent renders. And using the **dotenv** package to access environment variables was broken in the same way.

For `engine: julia` I also found that passing environment variables is unreliable and like for the python3 engine I experienced environment variable values being stuck after the first render. Using the Julia **DotEnv** package did seem to be reliable.
