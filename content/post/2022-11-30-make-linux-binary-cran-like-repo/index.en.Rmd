...
## [1] "source"
```
therefore, on Linux, `utils:::resolvePkgType()`{.r} always returns `"source"`. Hence, on Linux, `contrib.url()`{.r} always returns `/src/contrib` regardless of its `type` argument.

## Testing your CRAN-like repositories locally

You can either run a local web server or use the `file://...` URL notation as your `repos` global options setting (set with `option(repos = ...)` or as the `repos` argument to `install.packages()`{.r}).
