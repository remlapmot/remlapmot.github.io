...
tags:
  ...
  - Stan
subtitle: ''
summary: "Tips to help speed up Stan model compilation times, especially helpful for R package developers. Including, setting `MAKEFLAGS`, using `ccache`, and using `clang`."
...
Here's a quick test script to run to determine where the timings plateau to set the number. You can increase the number sequence up to the number of processing cores your machine has.
...
My MacBook Air has 10 cores, but only 4 of those are performance cores, so I set the number as 6 as that is where my timings plateaued and it leaves headroom for me inevitably checking my email during a compilation.
...
On my Windows machine this reduced the build from 3:55 to 1:15.
...
To enable `ccache`, on macOS/Linux this goes in _~/.R/Makevars_; on Windows it's _~/.R/Makevars.win_ (create the directory and file if they don't exist), set
...
I observe that switching from `gcc` to `clang` gives a noticeable speed up; the single core compile time dropped from 3 minutes 55 seconds to 3 minutes flat.

To do this you need to install `clang`. This is more involved on Windows than Linux because you install `clang` within RTools45, but it is possible.
...
## (Money no object) Big win: Switch to an Apple Silicon Mac
...
In summary, I have shown four big wins and a small win how to speed up compilation times of Stan models within an R package.
