...
Here's a quick script to test which model takes the longest to compile. Save it as say _test.sh_ at the top level of your repo and add `^test\.sh$` to your _.Rbuildignore_ file.
...
This reduced by ubuntu-latest run for r-release from 7 minutes 30 seconds to 4 minutes 49 seconds.

## (Money no object) Big win 4: Switch to an Apple Silicon Mac
...
## Big win 5: Linux users switch to `clang` version 18

In my WSL Ubuntu 24.04 I observe that switching from `gcc` to `clang` gives a noticeable speed up; the single core compile time dropped from 3 minutes 55 seconds to 3 minutes flat.
...
pacman -Syu # you may need to close and reopen the shell after this
...
Switch to `clang` in your _~/.R/Makevars_ file (if you're not using `ccache` delete that prefix)
...
and in your _~/.R/Makevars.win_ file on Windows
...
And Windows users will need to add the following to `PATH`
...
I believe you need `clang` version 18 to see the speedups (I had versions 18.1.3 on Ubuntu and 22.1.4 on Windows).

## Small win 1: WSL users should use the native file system

Within WSL it is possible to access files from within its native Linux filesystem, i.e., within `/home/user/...`, but it is also possible to access files on the Windows filesystem, e.g., in `/mnt/c/...`. I believe file operations are faster within `/home/user/...`.
...
I also wondered whether using R on Windows Subsystem for Linux would speed things up just by virtue of being on Linux. It did not, timings using `gcc` on Windows and WSL Ubuntu were essentially identical.
